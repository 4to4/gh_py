...
# create a dict for artists appearing on more than 50 rows
...
def get_args():
    ...
    parser.add_argument(
        ...
        help='Please provide a valid .csv file to write the output.'
        )
...
def get_fhs(input_file, output_file):
    try:
        if (os.path.isfile(input_file) == False):
            print(f"Err : File [{input_file}] does not exist. Please provide a valid file. Exiting.")
            exit()

        if(os.path.getsize(input_file) == 0 ):
            print(f"Err : File [{input_file}] is zero bytes. Please provide a valid file. Exiting.")
            exit()
    except (OSError):
